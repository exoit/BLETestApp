...
public class GenericAccessProfile {
	...
	static {
		...
		sUUIDNameMap.put(PERIPHERAL_PRIVACY_FLAG_UUID,
	...
	public static int readApperance(
			BluetoothGattCharacteristic bluetoothGattCharacteristic) {
		return ProfileUtils.convertByteArrayToInt(bluetoothGattCharacteristic
				.getValue());
	}

	public static boolean readPeripheralPrivacyFlag(
			BluetoothGattCharacteristic bluetoothGattCharacteristic) {
		return (bluetoothGattCharacteristic.getValue()[0] & 0x1) > 0;
	}

	public static String readReconnectionAddress(
			BluetoothGattCharacteristic bluetoothGattCharacteristic) {
		return ProfileUtils.convertByteArrayToMAC(bluetoothGattCharacteristic
				.getValue());
	}

	public static int[] readPeripheralPreferredConnectionParameters(
			BluetoothGattCharacteristic bluetoothGattCharacteristic) {
		int values[] = new int[4];
		values[0] = bluetoothGattCharacteristic.getIntValue(
				BluetoothGattCharacteristic.FORMAT_UINT16, 0);
		values[1] = bluetoothGattCharacteristic.getIntValue(
				BluetoothGattCharacteristic.FORMAT_UINT16, 2);
		values[2] = bluetoothGattCharacteristic.getIntValue(
				BluetoothGattCharacteristic.FORMAT_UINT16, 4);
		values[3] = bluetoothGattCharacteristic.getIntValue(
				BluetoothGattCharacteristic.FORMAT_UINT16, 6);
		return values;
	}
}
