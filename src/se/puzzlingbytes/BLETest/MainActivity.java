...
public class MainActivity extends Activity {
	...
	public void showDeviceServices(BluetoothDevice bluetoothDevice) {
		...
		getFragmentManager().beginTransaction()
				.replace(R.id.fragment_container, mServiceFragment)
				.addToBackStack(null).commit();
	}

	public void showServiceCharacteristics(
			List<BluetoothGattCharacteristic> charaList) {
		mCharaList = charaList;
		getFragmentManager().beginTransaction()
				.replace(R.id.fragment_container, mCharaFragment)
				.addToBackStack(null).commit();
	}

	public void showCharacteristicsValue(
			BluetoothGattCharacteristic bluetoothGattCharacteristic) {
		mBluetoothGatt.readCharacteristic(bluetoothGattCharacteristic);
		String name = ProfileUtils.lookupUUIDName(
				bluetoothGattCharacteristic.getUuid(), "Unknown");
		mDialogFragment = CharaValueDialogFragment.newInstance(name);
	...
	private final BluetoothGattCallback bluetoothGattCallback = new BluetoothGattCallback() {
		...
		public void onConnectionStateChange(BluetoothGatt gatt, int status,
			...
			if (BluetoothGatt.STATE_CONNECTED == newState
				...
				Log.d("Connection", "Status: " + status + " NewState: "
						+ newState);
			}
		}
			...
			if (BluetoothGatt.GATT_SUCCESS == status) {
				...
				if (BatteryProfile.BATTERY_SERVICE_UUID.equals(serviceUUID)) {
					if (BatteryProfile.BATTERY_LEVEL_UUID.equals(valueUUID)) {
						value = String.valueOf(BatteryProfile
								.readBatteryLevel(characteristic) + "%");
					}
				} else if (GenericAccessProfile.GENERIC_ACCESS_SERVICE_UUID
						.equals(serviceUUID)) {
					if (GenericAccessProfile.DEVICE_NAME_UUID.equals(valueUUID)) {
						value = GenericAccessProfile
								.readDeviceName(characteristic);
					} else if (GenericAccessProfile.APPEARANCE_UUID
							.equals(valueUUID)) {
						value = String.valueOf(GenericAccessProfile
								.readApperance(characteristic));
					} else if (GenericAccessProfile.PERIPHERAL_PRIVACY_FLAG_UUID
							.equals(valueUUID)) {
						value = String.valueOf(GenericAccessProfile
								.readPeripheralPrivacyFlag(characteristic));
					} else if (GenericAccessProfile.RECONNECTION_ADDRESS_UUID
							.equals(valueUUID)) {
						value = GenericAccessProfile
								.readReconnectionAddress(characteristic);
					} else if (GenericAccessProfile.PERIPHERAL_PREFERRED_CONNECTION_PARAMETERS_UUID
							.equals(valueUUID)) {
						value = Arrays
								.toString(GenericAccessProfile
										.readPeripheralPreferredConnectionParameters(characteristic));
					} else if (TXPowerProfile.TX_POWER_SERVICE_UUID
							.equals(serviceUUID)) {
						if (TXPowerProfile.TX_POWER_LEVEL_UUID
								.equals(valueUUID)) {
							value = String.valueOf(TXPowerProfile
									.readTxPowerLevel(characteristic) + "dBm");
						}
					}
				}
	...
	public void showDialogFragment(FragmentManager fm, DialogFragment dialog,
			String tag) {
